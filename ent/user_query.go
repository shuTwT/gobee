...
type UserQuery struct {
	...
	withAiChatSessions *AIChatSessionQuery
...
// QueryAiChatSessions chains the current query on the "ai_chat_sessions" edge.
func (_q *UserQuery) QueryAiChatSessions() *AIChatSessionQuery {
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withAiChatSessions: _q.withAiChatSessions.Clone(),
...
// WithAiChatSessions tells the query-builder to eager-load the nodes that are connected to
// the "ai_chat_sessions" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithAiChatSessions(opts ...func(*AIChatSessionQuery)) *UserQuery {
	query := (&AIChatSessionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withAiChatSessions = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withAiChatSessions; query != nil {
		if err := _q.loadAiChatSessions(ctx, query, nodes,
			func(n *User) { n.Edges.AiChatSessions = []*AIChatSession{} },
			func(n *User, e *AIChatSession) { n.Edges.AiChatSessions = append(n.Edges.AiChatSessions, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadAiChatSessions(ctx context.Context, query *AIChatSessionQuery, nodes []*User, init func(*User), assign func(*User, *AIChatSession)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(aichatsession.FieldUserID)
	}
	query.Where(predicate.AIChatSession(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.AiChatSessionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
